...
#test_word = ['Dr.MehwishHassan', 'DigitalLogicDesign', 'EE227', 'Batch2018' ]

#test_word = ['Ms.SabaRasheedMalik', 'ComputerOrganization', 'AssemblyLanguage', 'sectionB', 'Batch2017', 'Repeat']
'''test_word = [] 
...
for i in range(0, n): 
    ele = str(input()) 
    test_word.append(ele) # adding the element 
...
filestr=file1.read()
test_word=filestr.split(' ')
#print (filestr) 
file1.close()
#print

#test_word = [u'Google', u'business']
...
class Model(object):
    def __init__(self, is_training, config):
        ...
        self._targets = tf.placeholder(tf.int32, [batch_size, num_steps]) #??????x, y
...
def main(_):
    ...
    with tf.Graph().as_default(), tf.Session(config=config_tf) as session:
        ...
        newstr=str(' '.join(beams[0][1][1:]).encode('utf-8'))
        newstr=newstr.replace('UNK','Unknown')
        newstr=newstr.replace('END','')
        newstr=newstr.replace('PAD','')
        ...
        #print(newstr)
        keywords1 = []
        sentences = nltk.sent_tokenize(newstr) 
        #print(sentences)
        #print('...........')
        ...
        for i in range(len(keywords1)):
          ...
          if not(st1.isupper()):
            if (st1[0].isupper()): 
              newstr=newstr.replace(st1,stnew)
		

        newstr=newstr.replace("section","section ")
        newstr=newstr.replace("Batch","Batch ")
        newstr=newstr.replace("Thirdfloor","Third floor ")
        newstr=newstr.replace("Secondfloor","Second floor ")
        newstr=newstr.replace("Groundfloor","Ground floor ")		
        print(newstr)
        f.write(newstr)
        f.write('\n')
    f.close()
